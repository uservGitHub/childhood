...
public class BackMap() {
    companion object {
        const val CellWidth = 400
        const val CellHeight = 400
...
class PageBuffer(ctx: Context) {
    ...
    val size: Int get() = array.size
    //fun fromInd(key: Int) = if (hasLock) array[onlyInd] else array[key]
    fun fromInd(key: Int) = if (hasLock) array[onlyInd].apply { rawFile.push(key) } else array[key]
    fun lockInd(key: Int) {
        onlyInd = key
        hasLock = true
    }
